...
class LaskerMorrisPlayer:
    ...
    def make_move(self):
        """Generate the best move using alpha-beta pruning with progressive deepening."""
        ...
        while depth <= self.max_depth and not self.is_time_up():
            ...
            if move:
                best_move = move  # Store the best move found so far

            depth += 1  # Increment depth for deeper search

        if not best_move:
            return None

        # Handle move formatting based on phase
        if best_move[0] == "h":
